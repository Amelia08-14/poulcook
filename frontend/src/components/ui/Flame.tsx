/**
 * Flamme vivante — reprend l'élément flamme du logo Poulcook, animée plutôt
 * que statique. Icône Heroicons (MIT), pas un caractère emoji en costume
 * d'icône.
 */
export default function Flame({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={`flame-flicker ${className}`}>
      <defs>
        <linearGradient id="flame-gradient" x1="12" y1="2" x2="12" y2="22" gradientUnits="userSpaceOnUse">
          <stop stopColor="var(--color-gold)" />
          <stop offset="1" stopColor="var(--color-coral)" />
        </linearGradient>
      </defs>
      <path
        fill="url(#flame-gradient)"
        d="M12.963 2.286a.75.75 0 0 0-1.071-.136 9.742 9.742 0 0 0-3.539 6.176 7.547 7.547 0 0 1-1.705-1.715.75.75 0 0 0-1.152-.082A9 9 0 1 0 15.68 4.534a7.46 7.46 0 0 1-2.717-2.248ZM15.75 14.25a3.75 3.75 0 1 1-7.313-1.172c.628.465 1.35.81 2.133 1a5.99 5.99 0 0 1 1.925-3.545 3.75 3.75 0 0 1 3.255 3.717Z"
      />
    </svg>
  );
}
